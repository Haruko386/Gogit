...
- 面向正式发布的整体完成度：约 60%。
- P0 Prompt 协议恢复：已完成。
- P1 静态命令、参数元数据和仓库动态候选：进行中。
- P2 编辑体验已覆盖常用快捷键；基础历史持久化已经实现，配置仍待实现。
- P3 跨平台 CI 与 Release workflow：已建立，完整端到端验收和发布体验仍待补齐。

当前版本已经具备持久 PTY Shell、常用行编辑快捷键、跨会话命令历史、Git 静态候选、Tab 补全、参数值提示、可滚动候选窗口、branch/remote/tag/工作区文件动态候选、多行粘贴、Unicode 终端宽度处理、长命令水平视口、命令间空行，以及包含虚拟环境和当前目录的动态 Prompt。
...
以下按可独立验收的功能里程碑统计，而不是按代码行数估算。截至 2026-09-26，共 19 项：已完成 11 项，进行中 3 项，待开始 5 项。当前阶段应优先完成动态候选、解析能力和历史持久化完善；剩余静态 Git 命令计划集中放到后续的大 PR 中补齐。
...
- [x] 候选列表最多同时显示 6 条并随选择滚动，同时保留全部前缀匹配结果。
...
- [ ] **历史持久化完善**：跨会话加载、容量限制、敏感命令过滤和自动压缩已经实现；仍需完善多进程安全和关闭持久化的配置。
...
- remote 名称候选。
- tag 候选。
- `git add` 的已修改、已删除和未跟踪文件候选。
- 默认 `git restore` 的已修改和已删除跟踪文件候选；不会错误推荐未跟踪文件。
- 分支查询取消、超时和仓库目录切换。
- `switch`、`checkout`、`merge`、`rebase`、`reset`、`log`、`diff`、`pull`、`push`、`bisect`、`show`、`blame`、`shortlog`、`describe` 和 `reflog` 等上下文中的分支候选路由。
- 仓库候选在插入命令行时按 Shell 安全规则处理；普通名称保持原样，包含特殊字符的名称会被引用。
- 候选默认最多同时显示 6 条，上下选择时滚动窗口，但保留全部前缀匹配结果。
...
候选继续使用静态目录和仓库查询产生的稳定顺序，不根据用户历史动态调整，避免低频候选被隐藏或候选位置不断变化。
...
- 上下方向键浏览候选或当前会话历史；
- Tab 补全、候选选择和草稿恢复；
- Tab 补全完成后会在行尾按上下文自动添加空格，并避免重复已有空白；
- 候选列表通过最多 6 条的显示窗口滚动浏览，不会截断完整候选集合；
- 多行 bracketed paste；
...
- 内存历史最多保留 1000 条记录；
- 历史以 JSON Lines 格式保存在操作系统用户配置目录；
- Gogit 启动时加载历史，命令执行时以追加方式写入；
- 跳过空命令和连续重复命令；
- 包含 token、密码、secret 和 authorization 信息的命令不会持久化；
- 以空格或 Tab 开头的命令不会持久化；
- 损坏的单条记录不会阻止其他历史加载；
- 历史记录超过容量阈值后，通过临时文件压缩并保留最新记录；
- 历史读取或写入失败不会中断 Shell。

仍需完成：

- 完善多个 Gogit 进程同时追加和压缩时的安全性；
- 提供关闭历史持久化的配置。

验收标准：重启 Gogit 后可以浏览历史；历史文件损坏、不可写或超过容量限制时不影响 Shell 使用。
...
1. 动态 revision、commit 和更多路径上下文候选；
2. 支持 Git 全局 option、引号、路径和 Shell 命令边界；
3. 完善历史文件的多进程安全和关闭配置；
4. Prompt/显示配置；
5. 根据真实使用反馈按需补充静态 Git 命令；
