...
- P2 编辑体验已覆盖常用快捷键；历史持久化和配置仍待实现。
- P3 跨平台 CI 与 Release workflow：已建立，完整端到端验收和发布体验仍待补齐。

当前版本已经具备持久 PTY Shell、常用行编辑快捷键、会话内历史、Git 静态候选、Tab 补全、参数值提示、branch/remote/tag/工作区文件动态候选、多行粘贴、Unicode 终端宽度处理、长命令水平视口、命令间空行，以及包含虚拟环境和当前目录的动态 Prompt。
